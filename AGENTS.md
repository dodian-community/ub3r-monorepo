...
- `LegendsGuildGateService` now carries a local visual-face fallback for gate objects `2391`/`2392` at `2728/2729,3349` because some live databases are missing the matching `uber3_doors` rows; keep the Kotlin fallback and SQL seed aligned if you change those gate visuals.
