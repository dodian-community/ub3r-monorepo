...
class RootPlayerInfoIdleSyncTest {

    @Test
    fun `idle skip queues empty player sync template`() {
        val client = registerClient(1)
        ...
        try {
            ...
            runRootSync(listOf(client))
            drainOutbound(client)

            val cycle = runRootSync(listOf(client))
            val outbound = drainSingleOutbound(client)
            val payload = outbound.toByteArray()

            assertEquals(81, outbound.opcode)
            assertEquals(0, readBits(payload, 0, 1))
            assertEquals(0, readBits(payload, 1, 8))
            assertEquals(2047, readBits(payload, 9, 11))
            assertEquals(1, cycle.playerPacketsIdleTemplated)
            assertEquals(0, cycle.playerPacketsBuilt)
            assertEquals(0, cycle.playerPacketsSkipped)
            assertEquals(0, cycle.playerLocalScans)
    ...
    @Test
    fun `idle skip preserves retained locals and keeps sending across soak`() {
        val viewer = registerClient(1)
        val other = registerClient(2)
        val originalUpdateRunning = Server.updateRunning
        val originalItemManager = Server.itemManager
        ...
        try {
            Server.updateRunning = false
            Server.itemManager = testItemManager()
            runRootSync(listOf(viewer, other))
            drainOutbound(viewer)
            drainOutbound(other)

            repeat(200) {
                val cycle = runRootSync(listOf(viewer, other))
                val viewerOutbound = drainSingleOutbound(viewer)
                val otherOutbound = drainSingleOutbound(other)
                val viewerPayload = viewerOutbound.toByteArray()

                assertEquals(81, viewerOutbound.opcode)
                assertEquals(81, otherOutbound.opcode)
                assertEquals(0, readBits(viewerPayload, 0, 1))
                assertEquals(1, readBits(viewerPayload, 1, 8))
                assertEquals(0, readBits(viewerPayload, 9, 1))
                assertEquals(2047, readBits(viewerPayload, 10, 11))
                assertEquals(2, cycle.playerPacketsIdleTemplated)
                assertEquals(0, cycle.playerLocalScans)
                assertFalse(viewer.disconnected)
                assertTrue(viewer.lastIdlePlayerSyncSentAtMillis > 0L)

                viewerOutbound.releaseAll()
                otherOutbound.releaseAll()
            }
        } finally {
            Server.updateRunning = originalUpdateRunning
            Server.itemManager = originalItemManager
