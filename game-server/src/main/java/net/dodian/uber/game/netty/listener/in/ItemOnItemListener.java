 ...
 * <p>
 * The entire business logic from {@code ItemOnItem.ProcessPacket} has been
 * copied verbatim, with only the packet-parsing section rewritten to read
 * directly from the {@link ByteBuf}. This eliminates the legacy Stream and
 * allows us to delete the old handler once this compiles.
 */
...
public class ItemOnItemListener implements PacketListener {
    ...
    public void handle(Client client, GamePacket packet) throws Exception {
        ...
        if (usedWithSlot > 28 || usedWithSlot < 0 || itemUsedSlot > 28 || itemUsedSlot < 0) {
        ...
        for (int h = 0; h < Utils.herbs.length; h++) {
            if ((itemUsed == Utils.herbs[h] && otherItem == 227) || (itemUsed == 227 && otherItem == Utils.herbs[h])) {
                ...
                int xp = client.premium ? Utils.grimy_herbs_xp[h] : 0;
        ...
        if (checkPotionUsed && checkOtherPotionUsed) {
            boolean hasAllItem = true;
            for (int pIt : potionItems) {
                if (pIt == 269) {
                    if (!client.playerHasItem(269) && !client.playerHasItem(111)) hasAllItem = false;
                } else if (!client.playerHasItem(pIt)) hasAllItem = false;
            }
            if (hasAllItem) {
                if (client.getSkillLevel(Skill.HERBLORE) >= 88) {
                    ...
                    client.send(new SendMessage("You need level 88 herblore to mix a super combat potion!"));
                }
            } else {
                client.send(new SendMessage("You need a torstol herb or (unf) potion, super attack, strength and defence potion!"));
            }
        }
        /* Overload potion */
        ...
        if (checkPotionUsed && checkOtherPotionUsed) {
            boolean hasAllItem = true;
            for (int pIt : potionItems)
                if (!client.playerHasItem(pIt)) hasAllItem = false;
            if (hasAllItem) {
                if (client.getSkillLevel(Skill.HERBLORE) >= 93) {
                    ...
                    client.send(new SendMessage("You need level 93 herblore to mix an overload potion!"));
                }
            } else {
                client.send(new SendMessage("You need a coconut, super combat potion and a ranging potion!"));
            }
        }
    ...
    private void legacyTail(Client client, int itemUsed, int useWith, int otherItem, int itemUsedSlot, int usedWithSlot, boolean knife) {
        /* Mix dose potions */
        ...
        /* Knife on rainbow partyhat to split */
        ...
        /* Cracker + rainbow partyhat -> black partyhat */
        ...
        /* Knife on black partyhat to split */
        ...
        for (int d = 0; d < Constants.darttip.length; d++) {
            if ((itemUsed == Constants.darttip[d] || otherItem == Constants.darttip[d]) && (itemUsed == 314 || otherItem == 314)) {
                client.resetAction();
        ...
        for (int h = 0; h < Constants.heads.length; h++) {
            if ((itemUsed == Constants.heads[h] || otherItem == Constants.heads[h]) && (itemUsed == 53 || otherItem == 53)) {
                client.resetAction();
        ...
        for (int id = 0; id < Constants.logs.length; id++) {
            if ((itemUsed == Constants.logs[id] || otherItem == Constants.logs[id]) && knife) {
                ...
                client.sendFrame246(8870, 250, Constants.longbows[id]);
                client.sendFrame246(8869, 250, Constants.shortbows[id]);
        ...
        for (int id1 = 0; id1 < Constants.shortbow.length; id1++) {
            if ((itemUsed == Constants.shortbows[id1] || otherItem == Constants.shortbows[id1]) && (itemUsed == 1777 || otherItem == 1777)) {
                client.resetAction();
                if (client.getLevel(Skill.FLETCHING) < Constants.shortreq[id1]) {
                    client.send(new SendMessage("Requires level " + Constants.shortreq[id1] + " fletching"));
                    return;
                }
        ...
        for (int b2 = 0; b2 < Constants.longbows.length; b2++) {
            if ((itemUsed == Constants.longbows[b2] || otherItem == Constants.longbows[b2]) && (itemUsed == 1777 || otherItem == 1777)) {
                client.resetAction();
                if (client.getLevel(Skill.FLETCHING) < Constants.longreq[b2]) {
                    client.send(new SendMessage("Requires level " + Constants.longreq[b2] + " fletching"));
                    return;
                }
        ...
        /* Leather crafting menu */
        for (int h = 0; h < Constants.leathers.length; h++) {
            if ((itemUsed == 1733 || otherItem == 1733) && (itemUsed == Constants.leathers[h] || otherItem == Constants.leathers[h])) {
                client.craftMenu(h);
                client.cIndex = h;
            }
        }
