...
public class MagicOnNpc implements Packet {
    ...
    public void ProcessPacket(Client client, int packetType, int packetSize) {
        ...
        try {
            ...
            slayerTasks slayerTask = slayerTasks.getSlayerNpc(type);
            ...
            if (type == 3209 || type == 3204 || type == 3201) { //Brimhaven dungeon monster
                ...
                if (client.getLevel(Skill.SLAYER) < slayerRequired) {
                    client.send(new SendMessage("You need a slayer level of "+slayerRequired+" to harm this monster!"));
                    return;
                }
            }
            ...
            for (int i2 = 0; i2 < client.ancientId.length; i2++) {
                if (magicID == client.ancientId[i2]) {
                    if (!client.runeCheck(magicID)) break;
                    ...
                    if (client.getLevel(Skill.MAGIC) >= client.requiredLevel[i2]) {
                        ...
                        if (client.ancientType[i2] == 3) {
                            ...
                            client.setCurrentHealth(client.getCurrentHealth() + (int) (hit / 4));
