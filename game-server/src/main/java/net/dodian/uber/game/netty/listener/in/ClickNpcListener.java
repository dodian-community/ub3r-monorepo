...
import net.dodian.uber.game.content.npcs.action1.NpcAction1Dispatcher;
...
/**
 * Netty port of first-click NPC packet (opcode 155).
 * Logic is lifted from legacy ClickNpc class.
 */
@PacketHandler(opcode = 155)
public class ClickNpcListener implements PacketListener {

    static {
        PacketListenerManager.register(155, new ClickNpcListener());
    }

    @Override
    public void handle(Client client, GamePacket packet) {
        ByteMessage msg = ByteMessage.wrap(packet.getPayload());
        int npcIndex = msg.getShort(true, ByteOrder.LITTLE, ValueType.NORMAL);
        ...
        if (npcId == 2794) { // sheep shearing quick action
        ...
        // Defer remaining interaction until player reaches NPC
    ...
    private void performClick(Client client, Npc tempNpc) {
        if (!tempNpc.isAlive()) {
        ...
        if (npcId == 5809) {
            ...
            client.showNPCChat(3306, 590, new String[]{
        ...
        if (NpcAction1Dispatcher.tryHandle(client, tempNpc, tempNpc.getSlot())) {
            return;
        }
        ...
        } else if (npcId == 557) { // Monk stat boost
            tempNpc.requestAnim(5643, 0);
            for (int skill = 0; skill < 4; skill++) {
                skill = (skill == 3 ? 4 : skill);
                client.boost(5 + (int) (Skills.getLevelForExperience(client.getExperience(Objects.requireNonNull(Skill.getSkill(skill)))) * 0.15),
