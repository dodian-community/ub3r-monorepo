...
/**
 * Native Netty implementation of the massive "clicking buttons" opcode (185).
 * This is a straight port of the legacy {@code ClickingButtons.ProcessPacket}
 * method with minimal changes: the action button id is read directly from the
 * Netty {@link GamePacket} payload and the rest of the logic is unchanged.
 */
        ...
        switch (actionButton) {
            ...
            case 17100: // longrange (darts)
                ...
                if(actionButton == 1078 && client.autocast_spellIndex != -1) {
                    client.resetAttack();
                    //Swapping from magic to melee so stop combat!
                    client.autocast_spellIndex = -1; //Reset due to change of combat style
                }
                break;
            ...
            case 9158:
                ...
                if (client.NpcDialogue == 2) {
                    ...
                    client.NpcDialogueSend = false;
                                        ...
                                        client.send(new SetInterfaceWalkable(-1));
                    client.send(new RemoveInterfaces());
                } else if (client.NpcDialogue == 27) {
                    client.yellOn = false;
