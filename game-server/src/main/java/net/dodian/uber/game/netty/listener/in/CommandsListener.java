...
public class CommandsListener implements PacketListener {
    ...
    private String decodeCommand(GamePacket packet) {
        ...
        int length = 0;
        while (length < data.length && data[length] != 10 && data[length] != 0) {
            length++;
        }
        return new String(data, 0, length, java.nio.charset.StandardCharsets.ISO_8859_1);
    }
...
//                    }
            ...
            // Log unexpected errors during command execution to prevent crashes.
            logger.warn("Error executing command '{}' for player {}:", command, client.getPlayerName(), e);
            // Optionally, inform the user an error occurred, but avoid sending them the stack trace.
            // client.send(new SendMessage("An error occurred while executing the command."));
        }
    }
}
