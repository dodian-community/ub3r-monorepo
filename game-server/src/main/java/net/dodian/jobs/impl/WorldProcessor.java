        ...
        try {
            if (getWorldId() == 1) {
                try {
                    ...
                    String query = "UPDATE " + DbTables.GAME_WORLDS + " SET players = " + players + " WHERE id = " + getWorldId();
            ...
            try {
                String query = "SELECT * FROM " + DbTables.WEB_THREAD + " WHERE (forumid = '98' || forumid = '99' || forumid = '101') AND visible = '1' ORDER BY threadid DESC";
            ...
            try {
                for (Player p : PlayerHandler.players) {
                    if (p != null) {
                        Client c = ((Client) p);
                        String query = "SELECT * FROM " + DbTables.GAME_REFUNDS + " WHERE receiver='" + c.dbId + "' AND message='0' AND claimed IS NULL ORDER BY date ASC";
