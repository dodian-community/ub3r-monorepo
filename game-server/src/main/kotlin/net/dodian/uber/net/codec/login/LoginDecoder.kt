...
import net.dodian.cache.FileSystemConstants
import net.dodian.extensions.readString
import net.dodian.utilities.RsaManager
import net.dodian.utilities.StatefulFrameDecoder
import net.dodian.utilities.rsaKeys
...
class LoginDecoder(
    private val rsaManager: RsaManager = RsaManager()
    ...
    private fun decodePayload(ctx: ChannelHandlerContext, buffer: ByteBuf, out: MutableList<Any>) {
        ...
        val rsaKeyPair = rsaKeys
        if (rsaKeyPair != null) {
            var value = BigInteger(secure.array())
            value = value.modPow(rsaKeyPair.exponent, rsaKeyPair.modulus)
