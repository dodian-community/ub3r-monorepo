...
object NpcModuleDefinitionBuilder {
    ...
    fun fromModule(module: Any, explicitName: String, ownsSpawnDefinitions: Boolean): NpcContentDefinition {
        ...
        val pluginDefinition = readDslDefinition(moduleClass, module)
        if (pluginDefinition != null) {
            return pluginDefinition.toContentDefinition(
                explicitName = explicitName,
                ownsSpawnDefinitionsFlag = ownsSpawnDefinitions,
            )
        }
    ...
    private fun readDslDefinition(moduleClass: Class<*>, module: Any): NpcPluginDefinition? {
        val getter =
            moduleClass.methods.firstOrNull {
                it.parameterCount == 0 &&
                    (it.name == "getDefinition" || it.name == "getPlugin")
            } ?: return null
        return getter.invoke(module) as? NpcPluginDefinition
    }
