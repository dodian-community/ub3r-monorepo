...
class ShopCatalogTest {
    ...
    fun `definitions preserve migrated metadata`() {
        val premiumShop = ShopCatalog.find(20) ?: error("premium shop missing")
        val eventShop = ShopCatalog.find(55) ?: error("event shop missing")
        ...
        assertEquals(12, ShopCatalog.all().size)
    }
