...
object ShopRulesService {
    ...
    fun accessDeniedMessage(client: Client, shopId: Int): String? {
        val definition = ShopDefinitions.find(shopId) ?: return null
    ...
    @JvmStatic
    fun currencyItemId(shopId: Int): Int = ShopDefinitions.find(shopId)?.currencyItemId ?: 995

    @JvmStatic
    fun buyPrice(shopId: Int, itemId: Int, slot: Int): Int {
        val definition = ShopDefinitions.find(shopId)
    ...
    fun canSellItemToShop(shopId: Int, itemId: Int): Boolean {
        val definition = ShopDefinitions.find(shopId) ?: return false
    ...
    fun isDefaultStockItem(shopId: Int, itemId: Int): Boolean =
        ShopDefinitions.find(shopId)?.stock?.any { it.itemId == itemId } == true

    @JvmStatic
    fun isShopCurrency(itemId: Int): Boolean = ShopDefinitions.all().any { it.currencyItemId == itemId }
}
