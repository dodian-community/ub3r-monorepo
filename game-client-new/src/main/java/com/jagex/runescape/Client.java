...
import static net.dodian.client.ClientRSA.RSA_EXPONENT;
import static net.dodian.client.ClientRSA.RSA_MODULUS;
...
public final class Client extends RSApplet {
    ...
    private void login(final String playerUsername, final String playerPassword, final boolean recoveredConnection) {
        ...
        try {
            ...
            if (responseCode == 0) {
                ...
                this.stream.encrypt(RSA_MODULUS, RSA_EXPONENT);
    ...
    private Archive requestArchive(final int i, final String archive, final String group, final int j, final int k) {
        byte[] abyte0 = null;
        int l = 5;
        try {
        ...
        if (abyte0 != null) {
            System.out.println("[" + archive + "/" + group + "] Found cache locally!");
            return new Archive(abyte0);
        } else {
            System.out.println("[" + archive + "/" + group + "] Requesting archive from JagGrab...");
        }
        final int j1 = 0;
        while (abyte0 == null) {
            String s2 = "Unknown error";
            this.drawLoadingText(k, "Requesting " + archive);
            try {
                int k1 = 0;
                final DataInputStream datainputstream = this.openJagGrabInputStream(group + j);
                ...
                while (j2 < i2) {
                    ...
                    if (k3 != k1) {
                        this.drawLoadingText(k, "Loading " + archive + " - " + k3 + "%");
                    }
                    k1 = k3;
                }
